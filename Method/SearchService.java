...
public class SearchService {
    public static final String DateRegex = "^(\\d{4})-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[01])$";
    ...
    public ArrayList<Game> searchAndDisplay(Scanner scanner) throws Exception {
        ...
        do {
            ...
            System.out.println("Ấn 7 để tìm theo lượt tải");
            ...
            switch (search) {
                ...
                case 6:
                    ...
                    return result;
                case 7:
                    searchByDownload(scanner, result);
                    return result;
                default:
    ...
    private void searchByDownload(Scanner scanner, ArrayList<Game> result) {
        scanner.nextLine();
        System.out.println("Nhập số lượt tải ít nhất");
        String mindownload = validateInput(scanner, "^[0-3]?[0-9]{0,8}$|4{1}[0]{0,8}$", "Lượt tải không hợp lệ");
        System.out.println("Nhập số lượt tải nhiều nhất");
        String maxdownload = validateInput(scanner, "^[0-3]?[0-9]{0,8}$|4{1}[0]{0,8}$", "Lượt tải không hợp lệ");
        int min = Integer.parseInt(mindownload);
        int max = Integer.parseInt(maxdownload);

        for (Game game: gameList) {
            int download = Integer.parseInt(game.getTotalDownloads());
    ...
    private void searchByDate(Scanner scanner, ArrayList<Game> result) throws Exception {
        try {
            ...
            String oldestDate = validateInput(scanner, DateRegex, "Ngày phát hành không hợp lệ.");
            System.out.println("Đến ngày: ");
            String newestDate = validateInput(scanner, DateRegex, "Ngày phát hành không hợp lệ.");
